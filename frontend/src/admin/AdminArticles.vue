...
function uploadImages(e: any) {
  ...
  files.forEach(async (file) => {
    ...
    const res = await axios.post('/api/v1/admin/articles/upload-image', fd, { headers: { 'Content-Type': 'multipart/form-data' } })
    if (res.data.success) form.value.images.push(res.data.url)
  })
}
function uploadVideos(e: any) {
  const files = Array.from(e.target.files) as File[]
  files.forEach(async (file) => {
    const fd = new FormData()
    fd.append('video', file)
    const res = await axios.post('/api/v1/admin/articles/upload-video', fd, { headers: { 'Content-Type': 'multipart/form-data' } })
    if (res.data.success) form.value.videos.push(res.data.url)
  })
}
function removeImage(idx: number) {
  imgLoadingIdx.value = idx
  const url = form.value.images[idx]
  axios.post('/api/v1/admin/articles/delete-image', { url })
    .then(res => {
      if (res.data.success) {
    ...
    .catch(() => alert('刪除失敗'))
    .finally(() => { imgLoadingIdx.value = null })
}
function removeVideo(idx: number) {
  vidLoadingIdx.value = idx
  const url = form.value.videos[idx]
  axios.post('/api/v1/admin/articles/delete-video', { url })
    .then(res => {
      if (res.data.success) {
    ...
    .catch(() => alert('刪除失敗'))
    .finally(() => { vidLoadingIdx.value = null })
}
