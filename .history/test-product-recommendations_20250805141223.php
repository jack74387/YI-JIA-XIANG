...
try {
    ...
    $testProduct = Product::with('category')
        ->where('status', 'published') // 只測試上架商品
    ...
    if ($responseData['success']) {
        ...
        foreach ($recommendations as $index => $product) {
            $displayPrice = $product['price_large'] ?? $product['price_small'] ?? 0;
            echo "   " . ($index + 1) . ". {$product['name']}\n";
            echo "      - ID: {$product['id']}\n";
            echo "      - 分類: " . ($product['category_name'] ?? '無') . "\n";
            echo "      - 價格: \$" . number_format($displayPrice) . "\n";
        ...
        foreach ($recommendations as $product) {
            ...
            $recPrice = $product['price_large'] ?? $product['price_small'] ?? 0;
            if ($recPrice >= $minPrice && $recPrice <= $maxPrice) {
                $similarPrice++;
            }
        }
