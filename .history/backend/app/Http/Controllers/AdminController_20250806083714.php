...
class AdminController extends Controller
{
    ...
    private function extractPublicIdFromUrl($url)
    {
        ...
        if (preg_match('/\/image\/upload\/(.+)$/i', $url, $matches)) {
            ...
            if (!empty($cleanSegments)) {
                ...
                \Log::info('Extracted public_id using flexible parsing: ' . $publicId);
                return $publicId;
            }
        }
    ...
    public function deleteCloudinaryImage(Request $request)
    {
        $user = $request->user();
        if (!$user || !$user->is_admin) {
            \Log::warning('Cloudinary圖片刪除被拒絕：無權限', ['user_id' => $user->id ?? null]);
            return response()->json(['success' => false, 'message' => '無權限'], 403);
        }

        $imageUrl = $request->input('image_url');
        \Log::info('收到Cloudinary圖片刪除請求', [
            'admin_id' => $user->id,
            'image_url' => $imageUrl
        ]);

        if (!$imageUrl) {
            \Log::warning('Cloudinary圖片刪除失敗：未提供圖片URL');
            return response()->json(['success' => false, 'message' => '未提供圖片URL'], 400);
        }

        try {
            // 調用現有的刪除方法
            \Log::info('開始刪除Cloudinary圖片', ['image_url' => $imageUrl]);
            $this->deleteCloudinaryImageByUrl($imageUrl);
            ...
            \Log::info('Cloudinary圖片刪除成功', ['image_url' => $imageUrl, 'admin_id' => $user->id]);

            return response()->json([
                'success' => true,
                'message' => 'Cloudinary圖片刪除成功'
            ]);

        } catch (\Exception $e) {
            \Log::error('Cloudinary圖片刪除失敗: ' . $e->getMessage(), [
                'image_url' => $imageUrl,
                'admin_id' => $user->id,
                'exception' => $e->getTraceAsString()
            ]);
    ...
    public function deleteCloudinaryById(Request $request)
    {
        ...
        \Log::info('收到通過public_id刪除Cloudinary圖片請求', [
            'admin_id' => $user->id,
            'public_id' => $publicId
        ]);

        if (!$publicId) {
            \Log::warning('Cloudinary圖片刪除失敗：未提供public_id');
            return response()->json(['success' => false, 'message' => '未提供public_id'], 400);
        }
        ...
        try {
            ...
            \Log::info('開始刪除Cloudinary圖片', ['public_id' => $publicId]);
            
            // 直接使用public_id刪除
            $result = $cloudinary->uploadApi()->destroy($publicId, [
                'invalidate' => true // 清除CDN緩存
            ]);
            
            \Log::info('Cloudinary刪除結果', [
                'public_id' => $publicId,
                'result' => $result
            ]);
            ...
            \App\Models\OperationLog::create([
                'admin_id' => $user->id,
                'action' => 'delete_cloudinary_by_id',
                'ip' => $request->ip(),
                'user_agent' => $request->userAgent(),
                'data' => json_encode([
                    'public_id' => $publicId,
            ...
            if (isset($result['result'])) {
                if ($result['result'] === 'ok') {
                    \Log::info('Cloudinary圖片刪除成功', ['public_id' => $publicId]);
                    return response()->json([
                        'success' => true,
                        'message' => 'Cloudinary圖片刪除成功',
                        'result' => $result
                    ...
                    \Log::warning('Cloudinary圖片不存在', ['public_id' => $publicId]);
                    return response()->json([
                        'success' => true,
                        'message' => '圖片不存在（可能已被刪除）',
                        'result' => $result
                    ]);
                } else {
                    \Log::warning('Cloudinary刪除返回未知結果', ['public_id' => $publicId, 'result' => $result]);
                    return response()->json([
                        'success' => false,
                        'message' => '刪除失敗：' . ($result['result'] ?? '未知錯誤'),
                        'result' => $result
                    ]);
                }
            } else {
                \Log::error('Cloudinary刪除沒有返回result字段', ['public_id' => $publicId, 'response' => $result]);
                return response()->json([
                    'success' => false,
                    'message' => '刪除響應格式異常',
                    'result' => $result
