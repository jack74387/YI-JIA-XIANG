...
class AdminController extends Controller
{
    ...
    /**
     * 從 Cloudinary 刪除產品相關圖片
     */
    private function deleteProductImagesFromCloudinary($product)
    {
        try {
            $cloudinaryUrl = env('CLOUDINARY_URL');
            if (!$cloudinaryUrl) {
                \Log::warning('CLOUDINARY_URL not set, skipping image deletion');
                return;
            }
            ...
            if (!$urlParts || !isset($urlParts['user']) || !isset($urlParts['pass']) || !isset($urlParts['host'])) {
                \Log::error('Could not parse Cloudinary credentials from URL');
                return;
            }
            ...
            if (is_array($product->images)) {
                foreach ($product->images as $image) {
                    $imagePath = is_array($image) ? ($image['image_path'] ?? '') : $image;
            ...
            foreach ($imagesToDelete as $publicId) {
                try {
                    ...
                    \Log::info("Deleted Cloudinary image: $publicId", ['result' => $result]);
                } catch (\Exception $e) {
                    \Log::error("Failed to delete Cloudinary image: $publicId", ['error' => $e->getMessage()]);
                }
            }

        } catch (\Exception $e) {
            \Log::error('Error deleting images from Cloudinary', [
    ...
    /**
     * 從圖片路徑中提取 Cloudinary public_id
     */
    private function extractCloudinaryPublicId($imagePath)
    {
        if (empty($imagePath)) {
            return null;
        }

        // 如果是 Cloudinary URL，提取 public_id
        if (strpos($imagePath, 'cloudinary.com') !== false) {
            // URL 格式: https://res.cloudinary.com/cloud_name/image/upload/v123456789/folder/filename.ext
            $pattern = '/\/v\d+\/(.+?)(?:\.[^.]+)?$/';
            if (preg_match($pattern, $imagePath, $matches)) {
                return $matches[1]; // 返回 folder/filename
            }
        }

        // 如果是以 /https:// 開頭（錯誤格式），先修正
        if (strpos($imagePath, '/https://') === 0) {
            $imagePath = substr($imagePath, 1);
            return $this->extractCloudinaryPublicId($imagePath);
        }
