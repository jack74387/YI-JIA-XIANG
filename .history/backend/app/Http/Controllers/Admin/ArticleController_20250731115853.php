...
class ArticleController extends Controller
{
    ...
    // 圖片上傳 - 使用 Cloudinary Upload Presets (無權限限制)
    public function uploadImage(Request $request)
    {
        // 移除權限檢查，允許任何人上傳
        $user = $request->user();
        \Log::info('文章圖片上傳請求', ['user_id' => $user?->id, 'has_user' => !!$user]);
        ...
        try {
            ...
            // 使用環境變量配置 Cloudinary
            $cloudName = getenv('CLOUDINARY_CLOUD_NAME') ?: env('CLOUDINARY_CLOUD_NAME');
            $apiKey = getenv('CLOUDINARY_API_KEY') ?: env('CLOUDINARY_API_KEY');
            $apiSecret = getenv('CLOUDINARY_API_SECRET') ?: env('CLOUDINARY_API_SECRET');
    ...
    // 影片上傳 - 使用 Cloudinary Upload Presets (無權限限制)
    public function uploadVideo(Request $request)
    {
        // 移除權限檢查，允許任何人上傳
        $user = $request->user();
        \Log::info('文章影片上傳請求', ['user_id' => $user?->id, 'has_user' => !!$user]);
        
        $request->validate([
            'video' => 'required|mimetypes:video/mp4,video/quicktime,video/x-msvideo,video/webm|max:102400', // 100MB
        ]);
        ...
        try {
            ...
            // 使用 Upload Preset 上傳影片（注意：影片可能需要不同的 preset）
            $uploadedFile = $cloudinary->uploadApi()->upload($file->getRealPath(), [
                'upload_preset' => 'yijiaxiang', // 可以考慮為影片建立專用的 preset
                'resource_type' => 'video',
                'folder' => 'yijiaxiang/articles/videos',
                'context' => [
                    'alt' => 'Article video for 一佳香',
                    'caption' => 'Article video uploaded from admin panel'
                ],
                'tags' => ['video', 'article', 'yijiaxiang', 'admin-upload']
            ]);
