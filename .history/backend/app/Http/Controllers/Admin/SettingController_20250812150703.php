...
class SettingController extends Controller
{
    ...
    public function uploadHeroBgImage(Request $request)
    {
        ...
        // Debug: 檢查環境變數
        \Log::info('CLOUDINARY_CLOUD_NAME: ' . env('CLOUDINARY_CLOUD_NAME'));
        \Log::info('CLOUDINARY_API_KEY: ' . env('CLOUDINARY_API_KEY'));
        \Log::info('CLOUDINARY_API_SECRET: ' . env('CLOUDINARY_API_SECRET'));

        if (!env('CLOUDINARY_CLOUD_NAME') || !env('CLOUDINARY_API_KEY') || !env('CLOUDINARY_API_SECRET')) {
            \Log::error('Cloudinary 環境變數未正確加載');
            return response()->json([
                'success' => false,
                'message' => 'Cloudinary 環境變數未正確加載'
            ], 500);
        }
