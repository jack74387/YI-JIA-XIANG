...
class ArticleController extends Controller
{
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    // 如果都失敗，拋出原始錯誤
                    \Log::error('Cloudinary 資源刪除失敗（圖片和影片都嘗試過）', [
                        'public_id' => $publicId,
                        'image_error' => $imageError->getMessage(),
                        'video_error' => $videoError->getMessage()
                    ]);
                    throw new \Exception('無法刪除資源，既不是圖片也不是影片: ' . $imageError->getMessage());
                }
            }
            ...
            return response()->json([
                'success' => true,
                'message' => $resourceType === 'video' ? '影片刪除成功' : '圖片刪除成功',
                'public_id' => $publicId,
                'resource_type' => $resourceType,
                'result' => $result
            ]);
    ...
    /**
     * 刪除文章相關的所有 Cloudinary 資源
     */
    private function deleteArticleCloudinaryAssets($article)
    {
        try {
            $cloudName = getenv('CLOUDINARY_CLOUD_NAME') ?: env('CLOUDINARY_CLOUD_NAME', 'daeb3goxf');
            $apiKey = getenv('CLOUDINARY_API_KEY') ?: env('CLOUDINARY_API_KEY', '697592912781924');
            ...
            if (!$cloudName || !$apiKey || !$apiSecret) {
                throw new \Exception('Cloudinary 配置缺失');
            }
            ...
            // 刪除圖片
            if (!empty($article->images_public_ids)) {
                foreach ($article->images_public_ids as $publicId) {
                    if (!empty($publicId)) {
                        try {
                            $result = $cloudinary->adminApi()->deleteAssets([$publicId]);
                            $deletedAssets[] = ['type' => 'image', 'public_id' => $publicId, 'result' => $result];
                            \Log::info('文章圖片刪除成功', ['public_id' => $publicId]);
                        } catch (\Exception $e) {
                            \Log::warning('文章圖片刪除失敗', [
                                'public_id' => $publicId,
                                'error' => $e->getMessage()
                            ]);
                        }
            ...
            // 刪除影片
            if (!empty($article->videos_public_ids)) {
                foreach ($article->videos_public_ids as $publicId) {
                    if (!empty($publicId)) {
                        try {
                            $result = $cloudinary->adminApi()->deleteAssets([$publicId], ['resource_type' => 'video']);
                            $deletedAssets[] = ['type' => 'video', 'public_id' => $publicId, 'result' => $result];
                            \Log::info('文章影片刪除成功', ['public_id' => $publicId]);
                        } catch (\Exception $e) {
                            \Log::warning('文章影片刪除失敗', [
                                'public_id' => $publicId,
                                'error' => $e->getMessage()
                            ]);
                        }
            ...
            // 如果有舊格式的圖片（沒有 public_id），嘗試從 URL 提取並刪除
            if (!empty($article->images) && empty($article->images_public_ids)) {
                foreach ($article->images as $imageUrl) {
                    if (!empty($imageUrl)) {
                        try {
                            $publicId = $this->extractPublicIdFromUrl($imageUrl);
                            if ($publicId) {
                                $result = $cloudinary->adminApi()->deleteAssets([$publicId]);
                                $deletedAssets[] = ['type' => 'image_legacy', 'public_id' => $publicId, 'result' => $result];
                                \Log::info('文章舊格式圖片刪除成功', ['public_id' => $publicId, 'url' => $imageUrl]);
                            }
                        } catch (\Exception $e) {
                            \Log::warning('文章舊格式圖片刪除失敗', [
                                'url' => $imageUrl,
                                'error' => $e->getMessage()
                            ]);
                        }
            ...
            // 如果有舊格式的影片（沒有 public_id），嘗試從 URL 提取並刪除
            if (!empty($article->videos) && empty($article->videos_public_ids)) {
                foreach ($article->videos as $videoUrl) {
                    if (!empty($videoUrl)) {
                        try {
                            $publicId = $this->extractPublicIdFromUrl($videoUrl);
                            if ($publicId) {
                                $result = $cloudinary->adminApi()->deleteAssets([$publicId], ['resource_type' => 'video']);
                                $deletedAssets[] = ['type' => 'video_legacy', 'public_id' => $publicId, 'result' => $result];
                                \Log::info('文章舊格式影片刪除成功', ['public_id' => $publicId, 'url' => $videoUrl]);
                            }
                        } catch (\Exception $e) {
                            \Log::warning('文章舊格式影片刪除失敗', [
                                'url' => $videoUrl,
                                'error' => $e->getMessage()
                            ]);
                        }
            ...
            \Log::info('文章 Cloudinary 資源清理完成', [
                'article_id' => $article->id,
                'deleted_assets_count' => count($deletedAssets),
                'deleted_assets' => $deletedAssets
            ]);

        } catch (\Exception $e) {
            \Log::error('文章 Cloudinary 資源清理失敗', [
                'article_id' => $article->id,
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString()
            ]);
    ...
    private function extractPublicIdFromUrl($url)
    {
        ...
        // 例如: https://res.cloudinary.com/cloud_name/image/upload/v1234567890/folder/public_id.jpg
        $pattern = '/\/upload\/(?:v\d+\/)?(.+?)(?:\.[^.]+)?$/';
