                ...
                <div class="md:col-span-2">
                  ...
                  <div class="flex flex-wrap gap-4 mb-2">
                    <div v-for="(img, idx) in form.images" :key="img" class="relative">
                      ...
                      <button 
                        type="button" 
                        class="absolute top-0 right-0 bg-white bg-opacity-80 rounded-full p-1 hover:bg-red-100 transition-colors"
                        :class="{ 'opacity-50 cursor-not-allowed': deletingImages.has(img) }"
                        :disabled="deletingImages.has(img)"
                        @click="removeExtraImage(idx)"
                      >
                        {{ deletingImages.has(img) ? '⏳' : '✕' }}
                      </button>
                      <div v-if="deletingImages.has(img)" class="absolute inset-0 bg-black bg-opacity-50 rounded flex items-center justify-center">
                        <div class="text-white text-xs">刪除中...</div>
                      </div>
                    </div>
                    <button
...
async function removeExtraImage(idx) {
  ...
  if (editingProduct.value && editingProduct.value.id && form.value.images[idx]) {
    if (!confirm('確定要刪除這張圖片嗎？此操作將從 Cloudinary 永久刪除圖片。')) {
      return
    }
    
    const imageUrl = form.value.images[idx]
    deletingImages.value.add(imageUrl)
    ...
    try {
      ...
      if (response.data.success) {
        ...
        if (editingProduct.value) {
          editingProduct.value.images = [...form.value.images]
        }
        alert('圖片已從 Cloudinary 成功刪除')
      } else {
        alert('圖片刪除失敗：' + response.data.message)
      }
    } catch (error) {
      console.error('刪除圖片失敗:', error)
      alert('圖片刪除失敗：' + (error.response?.data?.message || '網路錯誤，請稍後再試'))
    } finally {
      deletingImages.value.delete(imageUrl)
    }
  } else {
    // 新增模式，直接從陣列移除
    form.value.images.splice(idx, 1)
...
async function removeImage() {
  ...
  if (editingProduct.value && editingProduct.value.id && form.value.image) {
    if (!confirm('確定要刪除主要圖片嗎？此操作將從 Cloudinary 永久刪除圖片。')) {
      return
    }
...
const deleteProduct = async (id) => {
  if (!confirm('確定要刪除這個商品嗎？此操作將同時刪除商品的所有圖片（包括 Cloudinary 上的圖片），且無法復原。')) return
...
const closeModal = () => {
  ...
  selectedFile.value = null
  uploadProgress.value = 0
  deletingImages.value.clear() // 清除刪除狀態
  form.value = {
    name: '',
    category_id: '',
    description: '',
