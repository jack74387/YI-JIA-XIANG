...
async function uploadVideos(e: any) {
  ...
  try {
    const res = await axios.get('/api/v1/admin/cloudinary-signature');
    signatureResponse = res.data;
    if (!signatureResponse.success) {
      throw new Error('無法取得上傳簽章');
    }
  } catch (error) {
    alert('影片上傳授權失敗，請稍後再試。');
    return;
  }

  const cloudinaryUrl = `https://api.cloudinary.com/v1_1/${signatureResponse.cloud_name}/video/upload`;
  ...
  for (const file of files) {
    ...
    formData.append('folder', 'yijiaxiang_videos');

    try {
      const res = await axios.post(cloudinaryUrl, formData);
      if (res.data.secure_url) {
        form.value.videos.push(res.data.secure_url);
      }
    } catch (error) {
      console.error(`影片 ${file.name} 上傳至 Cloudinary 失敗:`, error);
      alert(`影片 ${file.name} 上傳失敗。`);
    }
  }
  e.target.value = ''; // 清空 input
}
