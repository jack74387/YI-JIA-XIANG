          ...
          <div class="bg-white rounded-lg shadow p-6">
            ...
            <div class="h-64 flex items-end justify-between space-x-2">
              ...
              >
                ...
                <div
                  class="w-full bg-amber-200 rounded-t min-h-[4px]"
                  :style="{ height: Math.max(getBarHeight(day.revenue), 4) + 'px' }"
                ></div>
                <div class="text-xs text-gray-600 mt-1">NT$ {{ formatNumber(day.revenue) }}</div>
              </div>
              <!-- 當沒有數據時顯示提示 -->
              <div v-if="!dashboardData.sales_trend || dashboardData.sales_trend.length === 0" class="flex-1 text-center text-gray-500">
                暫無銷售數據
              </div>
            </div>
          </div>
