          ...
          <div class="mt-3">
            ...
            <form @submit.prevent="showEditModal ? updateProduct() : createProduct()">
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <!-- 包裝規格區域 -->
                <div class="md:col-span-2">
                  <label class="block text-sm font-medium text-gray-700 mb-2">包裝規格</label>
                  <div class="space-y-3">
                    <div v-for="(pkg, index) in form.package_info" :key="index" class="flex items-center space-x-3 p-3 bg-gray-50 rounded-md">
                      <div class="flex-1">
                        <input 
                          v-model="pkg.name" 
                          type="text" 
                          placeholder="包裝名稱 (如：600g 大包裝)"
                          class="w-full px-2 py-1 text-sm border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-amber-500 mb-1"
                        />
                        <input 
                          v-model="pkg.description" 
                          type="text" 
                          placeholder="包裝描述 (如：適合家庭分享)"
                          class="w-full px-2 py-1 text-sm border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-amber-500"
                        />
                      </div>
                      <button 
                    ...
                    <button 
                      type="button"
                      @click="addPackage"
                      class="w-full px-3 py-2 border border-dashed border-gray-400 rounded-md text-sm text-gray-600 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-amber-500"
                    >
                      + 添加包裝規格
                    </button>
                  </div>
...
const editProduct = (product) => {
  ...
  form.value = {
    ...
    package_info: Array.isArray(product.package_info) ? [...product.package_info] : []
  }
...
// 添加包裝規格
const addPackage = () => {
  form.value.package_info.push('')
}

// 移除包裝規格
const removePackage = (index) => {
  form.value.package_info.splice(index, 1)
}
