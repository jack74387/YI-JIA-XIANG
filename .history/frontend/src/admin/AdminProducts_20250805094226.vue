  ...
  if (editingProduct.value && editingProduct.value.id && form.value.images[idx]) {
    ...
    if (imageToDelete) {
      try {
        ...
        // 從待刪除列表中移除
        pendingImages.value = pendingImages.value.filter(img => img.url !== imageToDelete);
      } catch (error) {
        console.error('刪除Cloudinary圖片失敗:', error);
        // 即使刪除失敗也繼續，但提醒用戶
        alert('刪除圖片時發生錯誤，但將繼續操作');
      }
    }
...
async function removeImage() {
  ...
  if (editingProduct.value && editingProduct.value.id && form.value.image) {
    ...
    if (form.value.image) {
      try {
        // 調用刪除圖片API
        await deleteImageFromCloudinary(form.value.image);
        
        // 從待刪除列表中移除
        pendingImages.value = pendingImages.value.filter(img => img.url !== form.value.image);
      } catch (error) {
        console.error('刪除Cloudinary圖片失敗:', error);
        // 即使刪除失敗也繼續，但提醒用戶
        alert('刪除圖片時發生錯誤，但將繼續操作');
