...
async function handleExtraImagesUpload(e) {
  ...
  try {
    ...
    alert('圖片上傳授權失敗，請稍後再試。');
    return;
  }

  const cloudinaryUrl = `https://api.cloudinary.com/v1_1/${signatureResponse.cloud_name}/image/upload`;
  ...
  for (const file of files) {
    ...
    formData.append('folder', 'yijiaxiang');

    try {
      const res = await axios.post(cloudinaryUrl, formData);
      if (res.data.secure_url) {
        form.value.images.push(res.data.secure_url);
      }
    } catch (error) {
      console.error('額外圖片上傳至 Cloudinary 失敗:', error);
