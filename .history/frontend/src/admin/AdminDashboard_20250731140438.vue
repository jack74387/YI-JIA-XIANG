      ...
      <div v-else>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
          ...
          <div class="bg-white rounded-lg shadow p-6">
            ...
            <div v-if="dashboardData.sales_trend && dashboardData.sales_trend.length > 0" class="h-64 flex items-end justify-between space-x-2">
              <div
                v-for="(day, index) in dashboardData.sales_trend"
                :key="index"
                class="flex-1 flex flex-col items-center"
              >
                <div class="text-xs text-gray-500 mb-1">{{ formatDate(day.date) }}</div>
                <div
                  class="w-full rounded-t min-h-[4px] transition-all duration-300 hover:bg-amber-300"
                  :class="day.revenue > 0 ? 'bg-amber-400' : 'bg-gray-200'"
                  :style="{ height: Math.max(getBarHeight(day.revenue), 4) + 'px' }"
                  :title="`${formatDate(day.date)}: NT$ ${formatNumber(day.revenue)}`"
                ></div>
            ...
            <div v-else class="h-64 flex items-center justify-center text-gray-500">
              <div class="text-center">
                ...
                <p class="text-lg font-medium text-gray-600">暫無銷售數據</p>
                <p class="text-sm text-gray-500 mt-1">當有訂單後，這裡會顯示銷售趨勢圖表</p>
              </div>
            </div>
          </div>
...
const getBarHeight = (revenue) => {
  if (!dashboardData.value.sales_trend || !Array.isArray(dashboardData.value.sales_trend)) return 4
  const revenues = dashboardData.value.sales_trend.map(day => day.revenue || 0)
  const maxRevenue = Math.max(...revenues)
  if (maxRevenue === 0) return 4 // 即使沒有收入也顯示最小高度
  return Math.max(((revenue || 0) / maxRevenue) * 200, 4) // 最大高度 200px，最小高度 4px
}
