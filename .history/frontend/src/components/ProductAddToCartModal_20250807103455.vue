...
function getProductImage() {
  ...
  if (props.product.primary_image?.image_path) {
    const url = getImageUrl(props.product.primary_image.image_path)
    console.log('Using primary_image:', url)
    return url
  }
  ...
  if (props.product.images && props.product.images.length > 0) {
    ...
    if (typeof firstImage === 'string') {
      ...
      console.log('Using first image (string):', url)
      return url
    } else if (firstImage?.image_path) {
      const url = getImageUrl(firstImage.image_path)
      console.log('Using first image (object):', url)
      return url
    }
  }
  
  // 備用：使用 image 屬性（可能是字串路徑）
  if (props.product.image) {
    const url = getImageUrl(props.product.image)
    console.log('Using image field:', url)
    return url
  }
