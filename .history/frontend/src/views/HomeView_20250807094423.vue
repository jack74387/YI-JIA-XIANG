            ...
            <div class="showcase-content">
              ...
              <div class="product-display" v-if="featuredProducts.length > 0">
                <div class="main-product">
                  <img 
                    :src="getImageUrl(featuredProducts[0]?.image) || '/images/product-placeholder.jpg'" 
                    :alt="featuredProducts[0]?.name || '主打商品'"
                    class="main-product-image"
                  />
                  <div class="product-info-overlay">
                    <div class="product-name">{{ featuredProducts[0]?.name }}</div>
                    <div class="product-price">NT$ {{ formatPrice(featuredProducts[0]?.price_large) }}</div>
                  </div>
                </div>
                
                <!-- 次要產品預覽 -->
                <div class="secondary-products" v-if="featuredProducts.length > 1">
                  <div 
...
.hero-bg::before {
  ...
  background: 
    ...
    radial-gradient(ellipse at 60% 40%, rgba(0, 0, 0, 0.08) 0%, transparent 50%);
  z-index: 1;
}
...
.hero-bg::after {
  ...
  height: 120px;
  background: 
    linear-gradient(to top, rgba(250, 247, 240, 0.4), transparent),
    linear-gradient(45deg, rgba(250, 247, 240, 0.2) 0%, transparent 30%);
  z-index: 1;
  clip-path: polygon(
    0 100%, 
    100% 100%, 
