...
const loadProduct = async () => {
  ...
  try {
    ...
    // API 現在返回 { success: true, product: ... } 格式
    product.value = res.data.success ? res.data.product : res.data
    console.log('Loaded product:', product.value)
    console.log('Primary image:', product.value.primary_image)
    console.log('Images array:', product.value.images)
    console.log('Image field:', product.value.image)
  } catch (error) {
...
function addToCart(specData: any) {
  ...
  console.log('addToCart called with:', specData)
  console.log('Current product.value:', product.value)
  ...
  selectedProduct.value = {
    ...
    // 備用：如果以上都沒有，使用第一張可用圖片
    image: getAllImages()[0] || null
  }
  
  console.log('Selected product for modal:', selectedProduct.value)
  console.log('Selected product primary_image:', selectedProduct.value.primary_image)
