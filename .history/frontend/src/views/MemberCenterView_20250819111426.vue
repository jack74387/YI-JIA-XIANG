...
const calculateTotalSpent = (orders: any[]) => {
  return orders
    .filter(order => ['delivered', 'paid'].includes(order.status)) // 只計算有效訂單
    .reduce((total, order) => total + order.final_amount, 0); // 累加 final_amount
};
...
const loadMemberData = async () => {
  ...
  try {
    ...
    if (response.data.success) {
      ...
      // 重新計算累積消費金額
      statistics.value.total_spent = calculateTotalSpent(recentOrders.value);
    }
  } catch (error) {
