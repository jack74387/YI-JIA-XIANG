          ...
          >
            <div class="relative" @click="goToProduct(product.id)">
              <img
                ...
                @error="console.error('Image load error:', product.primary_image?.image_path, 'Final URL:', getImageUrl(product.primary_image?.image_path))"
              />
...
const loadProducts = async (page = 1) => {
  ...
  try {
    ...
    const response = await axios.get('/api/v1/proxy/products', { params })
    ...
    if (data.success) {
      ...
      // Debug: 檢查圖片路徑
      console.log('首個商品圖片資訊:', products.value[0]?.primary_image?.image_path)
      console.log('處理後的URL:', getImageUrl(products.value[0]?.primary_image?.image_path))
      // 不再強制排序，直接依 API 回傳順序
    }
  } catch (error) {
...
const loadCategories = async () => {
  try {
    ...
    // const data = await response.json()
    const response = await axios.get('/api/v1/proxy/categories')
    const data = response.data
...
const openAddToCart = (product: any) => {
  ...
  const price = getSpecPrice(product, spec);
  const weight = spec === 'large' ? '600g' : '300g'; // 根據規格更新重量
